...
#ifdef X86_2_LOONGARCH_INTRIN
// NOTE: Make sure every function defined in here has static linkage

// 4 * 4 = 16 instructions
static inline void transpose_kernel_4x4_lsx(
    const float* src,
    unsigned ld_src,
    float* dst,
    unsigned ld_dst) {
...
template <unsigned M>
static void transpose_kernel_mxn_lsx(
    unsigned N,
    const float* src,
    unsigned ld_src,
    float* dst,
...
// 8 * 5 = 40 instructions
static inline void transpose_kernel_8x8_lasx(
    const float* src,
    unsigned ld_src,
    float* dst,
    unsigned ld_dst) {
...
template <unsigned M>
static void transpose_kernel_mxn_lasx(
    unsigned N,
    const float* src,
    unsigned ld_src,
    float* dst,
  ...
  __m256i mask_v = _mm256_load_si256(
      reinterpret_cast<const __m256i*>(internal::lasx_ps_or_epi32_masks[N]));
  __m256 input[8];
  unsigned i;
  for (i = 0; i < M; ++i) {
  ...
  mask_v = _mm256_load_si256(
      reinterpret_cast<const __m256i*>(internal::lasx_ps_or_epi32_masks[M]));
  for (i = 0; i < N; ++i) {
    if (i < 4) {
...
// template <>
inline static void transpose_kernel_8x32_lasx(
    const uint8_t* src,
    unsigned ld_src,
    uint8_t* dst,
    unsigned ld_dst) {
...
// M + (M + 1) / 2 * 2 + (M + 3) / 4 * 4 + 2 * N instructions
template <unsigned M>
static void transpose_kernel_mxn_lasx_uint8(
    unsigned N,
    const uint8_t* src,
    unsigned ld_src,
    uint8_t* dst,
  ...
  __m256i mask_v = _mm256_load_si256(reinterpret_cast<const __m256i*>(
      internal::lasx_ps_or_epi32_masks[N / 4]));
  ...
  mask_v = _mm256_load_si256(reinterpret_cast<const __m256i*>(
      internal::lasx_ps_or_epi32_masks[M / 4]));
  for (i = 0; i < N; ++i) {
    if (i < 16) {
      if (i % 2 == 0)
