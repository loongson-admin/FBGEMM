...
void QuantizeLasx(
  ...
  if (rem > 0) {
    __m256i mask_v = _mm256_load_si256(reinterpret_cast<const __m256i*>(
        internal::lasx_ps_or_epi32_masks[rem]));
...
#define SPECIALIZE_QUANTIZELASX(T, LEGACY) \
  template void QuantizeLasx<T, LEGACY>(   \
      const float* src,                    \
      T* dst,                              \
      int len,                             \
      const TensorQuantizationParams& qparams);
SPECIALIZE_QUANTIZELASX(uint8_t, true)
SPECIALIZE_QUANTIZELASX(int8_t, true)
SPECIALIZE_QUANTIZELASX(uint8_t, false)
SPECIALIZE_QUANTIZELASX(int8_t, false)
#undef SPECIALIZE_QUANTIZELASX

template <typename T>
void NO_SANITIZE("address") FusedQuantizeDequantizeLasx(
    const float* src,
    float* dst,
    int len,
    const TensorQuantizationParams& qparams,
  ...
  for (; i < len / VLEN * VLEN; i += VLEN) {
    ...
    _mm_prefetch(reinterpret_cast<const char*>(src + i + VLEN));
    // _mm_prefetch(reinterpret_cast<const char*>(dst + i + VLEN), _MM_HINT_T0);
    _mm_prefetch(reinterpret_cast<const char*>(dst + i + VLEN));
...
#define SPECIALIZE_FUSEDDQLASX(T)               \
  template void FusedQuantizeDequantizeLasx<T>( \
      const float* src,                         \
      float* dst,                               \
      int len,                                  \
      const TensorQuantizationParams& qparams,  \
      float noise_ratio);
SPECIALIZE_FUSEDDQLASX(uint8_t)
SPECIALIZE_FUSEDDQLASX(int8_t)

#undef SPECIALIZE_FUSEDDQLASX
...
template <
    ...
    typename BIAS_TYPE,
    bool DIRECT>
void requantizeOutputProcessingLasx(
    uint8_t* out,
    const int32_t* inp,
  ...
  for (int i = block.row_start; i < block.row_start + block.row_size; ++i) {
    ...
    if (remainder > 0) {
      __m256i mask_v = _mm256_load_si256(reinterpret_cast<const __m256i*>(
          internal::lasx_ps_or_epi32_masks[remainder]));
...
template <
    ...
    int C_PER_G,
    typename BIAS_TYPE>
void requantizeOutputProcessingGConvLasx(
    uint8_t* out,
    const int32_t* inp,
    const block_type_t& block,
    int ld_out,
...
#define INSTANTIATE_REQUANTIZE_BIAS_TYPE(                  \
  ...
  template void FBGEMM_API requantizeOutputProcessingLasx< \
      A_SYM,                                               \
      B_SYM,                                               \
      Q_GRAN,                                              \
      BIAS,                                                \
  ...
  template void FBGEMM_API requantizeOutputProcessingLasx< \
      ...
      int ld_in,                                           \
      const requantizationParams_t<BIAS_TYPE>& r);         \
  template void requantizeOutputProcessingGConvLasx<       \
      A_SYM,                                               \
      B_SYM,                                               \
      Q_GRAN,                                              \
      BIAS,                                                \
...
#define INSTANTIATE_REQUANTIZE(A_SYM, B_SYM, Q_GRAN, BIAS, RELU)              \
  ...
  template void requantizeForFloatLasx<A_SYM, B_SYM, Q_GRAN, BIAS, RELU>(     \
      float* out,                                                             \
      const int32_t* inp,                                                     \
      const block_type_t& block,                                              \
      int ld_out,                                                             \
...
static inline uint16_t floatToHalf(float val) {
  ...
  return cpu_float2half_rn(val);
}
static inline float halfToFloat(uint16_t val) {
  // return _cvtsh_ss(val);
  return cpu_half2float(val);
}

template <typename InputType, int BIT_RATE>
void FloatOrHalfToFusedNBitRowwiseQuantizedSBHalfLasx(
  ...
  for (size_t row = 0; row < input_rows; ++row) {
    ...
    for (col = 0; col < input_columns / VLEN * VLEN; col += VLEN) {
      ...
      if (std::is_same<InputType, float>()) {
        ...
        in_v = _mm256_cvtph_ps(in_half_v);  // float16 -> float
...
void FloatOrHalfToFused8BitRowwiseQuantizedSBFloatLasx(
    const InputType* input,
    size_t input_rows,
    int input_columns,
    std::uint8_t* output) {
...
void FusedNBitRowwiseQuantizedSBHalfToFloatOrHalfLasx(
  ...
  if (BIT_RATE == 4 || BIT_RATE == 2) {
    ...
    vmask_load = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(
        internal::lasx_ps_or_epi32_combined_mask + NUM_OF_32BIT_PER_VLOAD +
...
void Fused8BitRowwiseQuantizedSBFloatToFloatOrHalfLasx(
    const std::uint8_t* input,
    size_t input_rows,
    int input_columns,
    OutputType* output) {
...
#define INSTANTIATE_QuantizationLasxFunctionsNBits(type, bit_rate)  \
  template void                                                     \
  FloatOrHalfToFusedNBitRowwiseQuantizedSBHalfLasx<type, bit_rate>( \
      const type* input,                                            \
      size_t input_rows,                                            \
      int input_columns,                                            \
      std::uint8_t* output);                                        \
  template void                                                     \
  FusedNBitRowwiseQuantizedSBHalfToFloatOrHalfLasx<type, bit_rate>( \
      const std::uint8_t* input,                                    \
      size_t input_rows,                                            \
      int input_columns,                                            \
      type* output);
...
// clang-format off
INSTANTIATE_QuantizationLasxFunctionsNBits(float, 2)
INSTANTIATE_QuantizationLasxFunctionsNBits(float, 4)
INSTANTIATE_QuantizationLasxFunctionsNBits(float, 8)
INSTANTIATE_QuantizationLasxFunctionsNBits(float16, 2)
INSTANTIATE_QuantizationLasxFunctionsNBits(float16, 4)
INSTANTIATE_QuantizationLasxFunctionsNBits(float16, 8)
// clang-format on
#undef INSTANTIATE_QuantizationLasxFunctionsNBits

#define INSTANTIATE_QuantizationLasxFunctions8Bits(type)                 \
  template void FloatOrHalfToFused8BitRowwiseQuantizedSBFloatLasx<type>( \
      const type* input,                                                 \
      size_t input_rows,                                                 \
      int input_columns,                                                 \
      std::uint8_t* output);                                             \
  template void Fused8BitRowwiseQuantizedSBFloatToFloatOrHalfLasx<type>( \
      const std::uint8_t* input,                                         \
      size_t input_rows,                                                 \
      int input_columns,                                                 \
      type* output);
    ...
    // clang-format off
INSTANTIATE_QuantizationLasxFunctions8Bits(float)
INSTANTIATE_QuantizationLasxFunctions8Bits(float16)
// clang-format on
#undef INSTANTIATE_QuantizationLasxFunctions8Bits
